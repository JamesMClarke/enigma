...
#   http://corylutton.com/enigma-python.html
#
...
def main():
    ...
    #Set all rotors to 0
    rotors = ["I II III","III I II","III II I", "I III II", "II I III","II III I"]
    found = False
    for rotor in rotors:
        print(rotor)
        rotor = rotor.split()
        print(rotor)
        machine.reset()
        machine.rotorsettings = [(rotor[0], 0), (rotor[1], 0), (rotor[2], 0)]
        counter = 0
        machine.rotors.clear()
        # Create each of the rotors
        for i in range(len(machine.rotorsettings)):
            machine.rotors.append(Rotor(machine.rotorsettings[i]))
        machine.print_setup()
